...
class ReturnsToScaleModel:
    ...
    def solve_envelopment(self, dmu_index: int) -> Tuple[float, float, str]:
        ...
        # Determine returns to scale type
        if abs(ccr_eff - 1.0) < 1e-6 and abs(sum_lambdas - 1.0) < 1e-6:
            rts_type = 'CRS'  # Constant Returns to Scale
        elif ccr_eff < 1.0 and sum_lambdas > 1.0:
            rts_type = 'NIRS'  # Non-Increasing Returns to Scale
        elif ccr_eff > 1.0 and sum_lambdas < 1.0:
            rts_type = 'NDRS'  # Non-Decreasing Returns to Scale
        else:
            rts_type = 'VRS'  # Variable Returns to Scale
    ...
    def solve_multiplier(self, dmu_index: int, epsilon: float = 1e-6) -> Tuple[float, float, str]:
        ...
        # Determine returns to scale based on u0
        if abs(u0) < 1e-6:
            rts_type = 'CRS'
        elif u0 < 0:
            rts_type = 'NIRS'
