...
class CostEfficiencyModel:
    ...
    def solve(self, dmu_index: int) -> Tuple[float, np.ndarray, np.ndarray]:
        ...
        """
        # Variables: [x_1, ..., x_m, lambda_1, ..., lambda_n]
        n_vars = self.n_inputs + self.n_dmus
        c = np.zeros(n_vars)
        c[:self.n_inputs] = self.input_costs[dmu_index, :]  # cost coefficients
        ...
        for i in range(self.n_inputs):
            A[i, i] = -1.0  # -x_i
            A[i, self.n_inputs:] = self.inputs[:, i]  # lambda coefficients
...
class RevenueEfficiencyModel:
    ...
    def solve(self, dmu_index: int) -> Tuple[float, np.ndarray, np.ndarray]:
        ...
        lambdas : np.ndarray
            Optimal intensity variables
        """
        # Variables: [y_1, ..., y_s, lambda_1, ..., lambda_n]
        n_vars = self.n_outputs + self.n_dmus
        c = np.zeros(n_vars)
        c[:self.n_outputs] = -self.output_prices[dmu_index, :]  # negative for maximization
        ...
        optimal_revenue = -result.fun  # negate because we minimized negative
